...
const YourBorrowsPage = (props: PopupProps) => {
  const currentLoans = props.loans.filter((l: Loan) => (l.state === '1' && l.borrower.toLowerCase() === props.accounts[0].toLowerCase()));
  const completeLoans = props.loans.filter((l: Loan) => (l.state !== '1' && l.borrower.toLowerCase() === props.accounts[0].toLowerCase()));
        ...
        <TableBody>
          {currentLoans.map((l: Loan) => {
            ...
            return (<TableRow key={l.cost} >
              ...
              <TableCell style={{ color: 'white' }}>{new Date(l.startTime * 1000).toLocaleString()}</TableCell>
              <TableCell style={{ color: 'white' }}>{l.loaner.slice(0, 20)}...</TableCell>
              <TableCell style={{ color: 'white' }}>
                {l.state === '1' ? <Button variant="contained" onClick={() => returnAsset(props.poolInst, props.accounts[0], props.loans.indexOf(l).toString())}>Return item<ArrowForwardIosIcon /></Button> : null}
              </TableCell>
            </TableRow>)
          })}
        </TableBody>
